...
export function buildZomatoOrder(cart, coupon) {
  ...
  const items = cart
    ...
    .map((item) => {
      const addonTotal = (Array.isArray(item.addons) ? item.addons : []).reduce((sum, addon) => {
        const [, addonPrice = "0"] = String(addon).split(":");
  ...
  const normalizedCoupon = typeof coupon === "string" ? coupon.toLowerCase() : "";
  let discount = 0;
  ...
  return {
    ...
    gst,
    discount,
    grandTotal: parseFloat(Math.max(subtotal + deliveryFee + gst - discount, 0).toFixed(2))
  };
}
